...
/***/ function(module, exports, __webpack_require__) {
	...
	var _loadimage = __webpack_require__(6);
	...
	var cursor = function cursor(imageLeft, imageRight) {
	  var arrowRight = new Image();
	  var arrowLeft = new Image();
	  var mouse = (0, _jquery2.default)('<figure></figure>');

	  (0, _loadimage2.default)(imageRight).then(function (img) {
	    arrowRight = img;
	  });

	  (0, _loadimage2.default)(imageLeft).then(function (img) {
	    arrowLeft = img;

	    mouse = (0, _jquery2.default)('<figure></figure>');
	    (0, _jquery2.default)('body').append(mouse);
	...
	function init(element, imgLeft, imgRight) {
	  var crs = cursor(imgLeft, imgRight);

	  var leave = element.asEventStream('mouseleave');
	  var enter = element.asEventStream('mouseenter');
	  var positionStream = element.asEventStream('mousemove');
	  var directionStream = element.asEventStream('mousemove').map(function (x) {
	    return getDirection(x.pageX, x.pageY, x.currentTarget.width, x.currentTarget.offsetLeft);
	  }).skipDuplicates();

	  hideSystemCursor(element);

	  enter.onValue(function () {
	    crs.show();
	  });

	  leave.onValue(function () {
	    crs.hide();
	  });

	  directionStream.onValue(function (move) {
	    crs.updateDirection(move);
	  });

	  positionStream.onValue(function (move) {
	...
	function getMaxDimensions(images) {
	  var dimX = images.map(function (img) {
	    return img.naturalWidth;
	  });
	  return dimX.reduce(function (prev, curr) {
